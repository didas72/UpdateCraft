...
def find_highest_common_version(local_mods: list[ModData]) -> str:
	version_sets: list[set[str]] = [(mod.compatible_game_versions if get_mod_versions(mod) else set()) for mod in local_mods]
...
def get_local_mod_data(mod_path: str) -> ModData | None:
	...
	with zipfile.ZipFile(mod_path, "r") as file:
		if "fabric.mod.json" in file.namelist():
			...
			mod_data = ModData(mod_info["name"], mod_info["version"], "fabric")
			return mod_data
		else:
			return None
